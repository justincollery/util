...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      // Fetch user's bills from the API
      fetchBills();
                      ...
                      <div key={bill.id} className={styles.billCard}>
                        ...
                        <div className={styles.billDetails}>
                          <div className={styles.billInfo}>
                            <p><strong>Bill Date:</strong> {bill.extractedData?.billDate || bill.billDate}</p>
                            <p><strong>Amount:</strong> {bill.processingStatus === 'processing' 
                              ? 'Processing...' 
                              : bill.extractedData?.costs?.totalAmount 
                                ? `€${bill.extractedData.costs.totalAmount}`
                                : `€${bill.amount?.toFixed(2) || '0.00'}`}
                            </p>
                            {bill.extractedData?.supplier && (
                              <p><strong>Supplier:</strong> {bill.extractedData.supplier}</p>
                            )}
                            {bill.extractedData?.units?.consumed && (
                              <p><strong>Usage:</strong> {bill.extractedData.units.consumed} {bill.extractedData.units.unit}</p>
                            )}
                            {bill.fileName && (
